...
<nav class="hidden md:flex fixed top-0 left-0 right-0 h-16 bg-(--color-bg) items-center justify-between px-6 z-50">
	<div class="flex items-center gap-2 font-semibold">
		<span class="text-(--primary-color)">
			<svg xmlns="http://www.w3.org/2000/svg" width="auto" height="20" viewBox="0 0 89 22" class="text-(--color-avans-logo)"
				><path
					fill="currentColor"
	...
	<div class="flex gap-8">
		{#each items as item}
          ...
          {$page.url.pathname === item.href ? 'text-(--primary-color)' : 'text-(--primary-color) hover:text-(--color-hyperlink)'}"
			>
				{item.label}
